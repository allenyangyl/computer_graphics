...
Spectrum PathTracer::trace_ray(const Ray &r) {
  ...
  //InfiniteHemisphereLight light(Spectrum(0.5, 0.5, 0.5));
  //DirectionalLight light(Spectrum(0.5, 0.5, 0.5), Vector3D(1.0, -1.0, 0.0));
  for (SceneLight *light : scene->lights) {
      ...
      for (int i=0; i<num_light_samples; i++) {
          ...
          Ray ray = Ray(o, dir_to_light);
          ray.max_t = dist_to_light;
          if(!bvh->intersect(ray)){
              Lo += light_L * f * cos_theta * (1.0 / pdf);
          }
      }
      ...
      Spectrum Lo_;
      Vector3D w_in_sample;
      for (int i=0; i<num_light_samples; i++) {
          Spectrum f_sample = isect.bsdf->sample_f(w_out, &w_in_sample, &pdf);
          double terProb = 1.0 - f_sample.illum();
          if((double)(std::rand()) / RAND_MAX > terProb && r.depth < max_ray_depth){
              double cos_theta;
              if(isect.bsdf->is_delta()){
                  cos_theta = 1.0;
              }
              else{
                  cos_theta = std::max(0.0, w_in_sample[2]);
              }
              Vector3D dir_to_object = o2w * w_in_sample;
              Vector3D o = hit_p + EPS_D * dir_to_object;
              Ray ray = Ray(o, dir_to_object);
              ray.depth = r.depth + 1;
              Spectrum trace = trace_ray(ray);
              Lo_ += (f_sample * trace * cos_theta) * (1 / pdf / (1 - terProb));
          }
      }

      Lo *= 1.0 / num_light_samples;
      Lo_ *= 1.0 / num_light_samples;
      L_out += Lo + Lo_;
  }
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  for (int i = 0; i < ns_aa; i++){
      ...
      double x_coor = (x + sample.x) / sampleBuffer.w;
      double y_coor = (y + sample.y) / sampleBuffer.h;
      Vector2D p = Vector2D(x_coor, y_coor);
